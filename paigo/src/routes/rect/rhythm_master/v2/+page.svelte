<script lang="ts">
	...
	function computePassedWidthPx(el: HTMLElement): number {
		const viewport = viewportEl;
		if (!viewport) return 0;
		const ph = playheadEl;
		const targetLeft = ph?.getBoundingClientRect().left ?? viewport.getBoundingClientRect().left;
		const noteRect = el.getBoundingClientRect();
		const passed = Math.min(noteRect.width, Math.max(0, targetLeft - noteRect.left));
		return passed;
	}

	function updateDimsAll() {
		if (!staffEl) return;
		const notes = staffEl.querySelectorAll('.note');
		notes.forEach((n) => {
			const width = computePassedWidthPx(n as HTMLElement);
			const dim = (n as HTMLElement).querySelector('.dim') as HTMLElement | null;
			if (dim) dim.style.width = `${width}px`;
		});
	}
	...
	function endHold(kind: TargetKind) {
		...
		if (held + TOL_MS >= expected) {
			...
			if (el) {
				const dimEl = el.querySelector('.dim') as HTMLElement | null;
				if (dimEl) dimEl.style.width = '0px';
			}
		}
	}
	...
	$effect(() => {
		if (player.holding && player.expectedMs > 0) {
			...
			const tick = () => {
				...
				// Update dim overlays for all notes to reflect current playhead position
				updateDimsAll();
