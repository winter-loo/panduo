...
export class NoteSpan extends Element {
  ...
  show(): this {
    ...
    if (this.innerWidth <= 0) {
      this.innerWidth = this.getEffectiveInnerWidth();
    }
  ...
  private updateDomDimensions(): void {
    ...
    const innerRect = group.querySelector('.span .inner') as SVGRectElement | null;
    if (innerRect) {
      innerRect.setAttribute('width', `${innerWidth}`);
    }

    const outerRect = group.querySelector('.span .outer') as SVGRectElement | null;
    if (outerRect) {
      const outerWidth = this.computeOuterWidth(innerWidth);
      if (outerWidth > 0) {
  ...
  private getEffectiveInnerWidth(): number {
    ...
    if (this.innerWidth > 0) {
      return Math.min(this.innerWidth, maxInner);
    }
    return Math.max(maxInner, this.getMinInnerWidth());
  }

  private computeOuterWidth(innerWidth: number): number {
    const slw = this.staffLineWidth || this.config.get('Stave.style.lineWidth');
    const { width } = this.bounds;
    const maxOuterWidth = Math.max(0, width - slw);
    const maxInnerWidth = this.getMaxInnerWidth();
    if (innerWidth <= 0 || maxInnerWidth <= 0) return 0;
    const expansion = Math.min(1, innerWidth / maxInnerWidth);
    return maxOuterWidth * expansion;
  }
